...
@Pasted("From Sigma Remake")
@SuppressWarnings("unused")
@UtilityClass
public final class CookieLoginUtil {
    ...
    public static LoginData loginWithCookie(File cookieFile) {
        try {
            ...
            String location = getNextLocation(url("https://sisu.xboxlive.com/connect/XboxLive/?state=login&cobrandId=8058f65d-ce06-4c30-9559-473c9275a65d&tid=896928775&ru=https%3A%2F%2Fwww.minecraft.net%2Fen-us%2Flogin&aid=1142970254"), "PHPSESSID=0");
            String location2 = getNextLocation(url(location.replace(" ", "%20")), cookie);
            String location3 = getNextLocation(url(location2.replace(" ", "%20")), cookie);
            ...
            HttpsURLConnection connection = (HttpsURLConnection) url("https://api.minecraftservices.com/minecraft/profile").openConnection();
            connection.setRequestMethod("GET");
            connection.setRequestProperty("Authorization", "Bearer " + mcToken);
            JsonObject profileResponse = GSON.fromJson(IOUtils.toString(connection.getInputStream(), StandardCharsets.UTF_8), JsonObject.class);
    ...
    public static String postExternal(@Language("http-url-reference") final String url, final String post, final boolean json) {
        try {
    ...
    private static @NotNull HttpsURLConnection postConnection(@Language("http-url-reference") String url, String post, boolean json) throws IOException {
        final HttpsURLConnection connection = (HttpsURLConnection) url(url).openConnection();
